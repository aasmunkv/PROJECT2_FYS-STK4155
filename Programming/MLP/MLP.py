...
class neuralNetwork:
    def __init__(self, dimensions, activation = "exponential", learning_rate = 0.04, loss_function = "accuracy", model = "regressor", initialization = "standard"):
        """Constructor method."""
    ...
    def change_network(self):
        self.network = np.copy(self.best_network)    
    ...
    def exponential_derivative(self, z):
        ...
        """
        a = self.exponential_activation(z)
        return a*(1-a)
    ...
    def feedForward(self, input):
        """
        A method that takes an input vector and feeds it through the neural 
        network. Returns activations from each layer. 
        """
    ...
    def backpropagation_multiple_layers_batch(self, inp, target, batch_size):
        """
        A method to train the MLP using backpropagation. Calculate the predicted 
        outputs and activations by feeding the inputs forward.
        """
        gradient = []
        bias = []
        batch_size = len(inp)
    ...
    def train(self, train, train_targets, test, test_targets, partition_sizes= [0.8, 0.1, 0.1], epochs = 150, plot_training_results = False, verbose=False):
        """
        A method that takes a dataset, processes it, and trains until 
        convergence criterium is met.
        """
        training_accuracy = np.zeros(epochs)
        test_accuracy = np.zeros(epochs)
        for k in range(epochs):
            self.train_method(train, train_targets)
            training_accuracy[k], diff_training, r2 = self.loss_function(train, train_targets)
            test_accuracy[k], diff_test, r2 = self.loss_function(test, test_targets)

            if(test_accuracy[k] > self.best_accuracy):
                #Keep a copy of the best performing network
                self.best_accuracy = test_accuracy[k]
                self.best_network = np.copy(self.network)
                self.best_epoch = k
            if(verbose):
                if(self.model_type == "classifier"):
                    ...
                    print("Iteration %i, loss = %f, R2 = %f" % (k, test_accuracy[k], r2))
    ...
    def accuracy_score(self, inputs, targets):
        """
        Calculates the accuracy score by counting the number of correct 
        predicitons devided by the number of predictions.     
        """
        N = len(inputs)
        pred = [0]*N
    ...
    def mean_squared_error(self, inputs, targets):
        """
        An alternative loss function, intended for regression use.     
        """
        predictions = np.zeros(len(inputs))
        for i in range(len(predictions)):
            predictions[i] = self.predict(inputs[i])
        diff = predictions - targets    
        return mean_squared_error(targets, predictions), diff, r2_score(targets, predictions)   
    ...
    def batch_train(self, train, train_targets, test, test_targets, batch_size, partition_sizes = [0.7, 0.2, 0.1], epochs = 150, plot_training_results = True, shuffle=True, verbose = False):
        """
        Batch training method. Picks a random sized batch. Calculates the 
        average value of n vectors with average targets.
        ...
        for k in range(epochs):
            ...
            if(flag):
                ...
                self.backpropagation_multiple_layers_batch(inp, target, batch_size)  
            training_accuracy[k], diff, r2 = self.loss_function(train, train_targets)
            test_accuracy[k], diff, r2 = self.loss_function(test, test_targets)
            if(test_accuracy[k] > self.best_accuracy):
                #Keep a copy of the best performing network
                self.best_accuracy = test_accuracy[k]
                self.best_network = np.copy(self.network)
    ...
    def split_data(self, inputs, targets, pratition_sizes):
        """
        Takes a set of inputs and outputs, shuffles them and splits them into 
